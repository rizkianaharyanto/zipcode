...
class Kab extends Controller
{
    public function search(Request $request)
    {
        ...
        foreach ($sample_excel as $excel) {
            // dd($excel);
            array_push($merge, $this->search_json($excel['Kabupaten'], $excel, $villages));
        }
        ...
        fclose($output);
        // return $out;
        return view('welcome', ['nama' => $request->outputfix,])->with('kab', 'done!');
    }
    ...
    public function search_json($snitch, $post, $seeker)
    {
        ...
        foreach ($seeker as $index => $data) {
            if (strlen($data['code']) == 5) {
                // dd("ya");
                // dd("KAB. ".$snitch_up);
                if (str_replace(" ", "", strtoupper($data['name'])) == "KAB.".$snitch_up && $post['code'] == substr($data["code"], 0, 2)) {
