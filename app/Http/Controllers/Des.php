...
class Des extends Controller
{
    public function search(Request $request)
    {
        ...
        foreach ($sample_excel as $excel) {
            // dd($excel);
            array_push($merge, $this->search_json($excel['Desa'], $excel, $villages));
        }
        ...
        fclose($output);
        // return $out;
        return view('welcome', ['nama' => $request->outputfix,])->with('des', 'done!');
    }
    ...
    public function search_json($snitch, $post, $seeker)
    {
        ...
        foreach ($seeker as $index => $data) {
            if (strlen($data['code']) == 13) {
                // dd("ya");
                // dd("KAB. ".$snitch_up);
                if (str_replace(" ", "", strtoupper($data['name'])) == $snitch_up && $post['code'] == substr($data["code"], 0, 8)) {
                    // return str_replace(" ", "", strtoupper($data['name']));
                    $hasil = array_merge(
                        array("district_id" => $post["kec"]),
